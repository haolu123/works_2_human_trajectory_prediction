...
class IRL:
    ...
    def get_mean_features(self,paths):
        ...
        for path in paths:
            for s in range(len(path)):
                ...
                f = self.features_get.get_features(s, a_p, a_c)
                sum_f += f
                count_f += 1
    ...
    def get_reward(self):
        ...
        R = np.zeros(n_position, n_action)
        
        position_map = self.feature_map[0] @ self.theta[:4]
        action_map = self.feature_map[1] @ self.theta[4:]
################################################################        
        p_map_v = np.tile(position_map.reshape(-1),(1,n_action))
        a_map_v = action_map.reshape(-1)
